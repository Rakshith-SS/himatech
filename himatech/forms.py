...
from himatech.models import User
...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        ...
        if re.search('^\d', username.data):
            raise ValidationError('Username cannot begin with a number')
...
class ChangeProfilePicture(FlaskForm):
    ...
    def validate_picture(self, picture):
        if len(picture.data.read()) < max_file_size:
            picture.data.seek(0)
        else:
            raise ValidationError('Image file should be less than 2 MB')
...
class UpdateAccountInfo(FlaskForm):
    ...
    def validate_username(self, username):
        ...
        if re.search(namePattern, username.data):
            ...
            raise ValidationError(f'{username.data} is not valid a username')
        if re.search('^\d', username.data):
            raise ValidationError('Username cannot begin with a number')
...
class CheckOut(FlaskForm):
    ...
    pincode = SelectField(
                        'Pincode',
                        choices=["560008", "560025", "560026"],
                        validate_choice=True
                        )
    city = SelectField(
                        'City',
                        choices=["Indirangar", "Richmond", "Shantinagar"],
                        validate_choice=True
                        )
